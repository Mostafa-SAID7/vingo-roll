...
export const Route = createFileRoute("/inspiration/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      ...pageHead({
        title: `${post.title} | Vingo Roll Inspiration`,
    ...
    <>
        ...
        <div className="absolute inset-x-0 bottom-0">
          <div className="container-page pb-12">
            <p className="eyebrow text-white/70">
              {post.style} · {post.room.replace(/-/g, " ")}
            </p>
      ...
      <Section className="bg-card">
        ...
        <div className="mt-10 flex flex-wrap gap-3">
          <Button asChild>
            <Link to="/swatches">Order these swatches</Link>
          </Button>
          <Button asChild variant="outline">
            <Link to="/inspiration">Back to gallery</Link>
          </Button>
        </div>
      </Section>
    </>
