...
export const Route = createFileRoute("/quote")({
  head: () =>
    pageHead({
      title: "Quick Quote Builder | Vingo Roll",
      description:
        "Build an indicative quote for made-to-measure curtains, shades and blinds in under a minute.",
      path: "/quote",
    }),
  component: Page,
});
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        description="An indicative range to help you plan. Final pricing always follows a confirmed measurement — this figure is a mock estimate, not an offer."
      />
      <Section>
        <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr]">
          ...
          >
            <div className="grid gap-5 sm:grid-cols-2">
              <div>
                ...
                <Select value={treatment} onValueChange={setTreatment}>
                  <SelectTrigger id="q-treatment" className="mt-2">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {categories.map((c) => (
              ...
              <div>
                ...
                <Select value={room} onValueChange={setRoom}>
                  <SelectTrigger id="q-room" className="mt-2">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {rooms.map((r) => (
              ...
              <div>
                <Label htmlFor="q-width">Approx. width (in)</Label>
                <Input
                  id="q-width"
                  type="number"
                  min={12}
              ...
              <div>
                <Label htmlFor="q-height">Approx. height (in)</Label>
                <Input
                  id="q-height"
                  type="number"
                  min={12}
              ...
              <div>
                ...
                <Select value={tier} onValueChange={setTier}>
                  <SelectTrigger id="q-tier" className="mt-2">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {tiers.map(([id, label]) => (
              ...
              <div>
                ...
                <Select value={lining} onValueChange={setLining}>
                  <SelectTrigger id="q-lining" className="mt-2">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                <Label htmlFor="q-qty">Number of windows</Label>
                <Input
                  id="q-qty"
                  type="number"
                  min={1}
            ...
            <div className="flex items-center gap-3">
              <Checkbox
                id="q-motor"
                checked={motorized}
                onCheckedChange={(v) => setMotorized(Boolean(v))}
              />
              <Label htmlFor="q-motor" className="font-normal">
                Add motorisation
              </Label>
            </div>
            ...
            {sent ? (
              <p role="status" className="text-muted-foreground text-sm">
                Thank you — a specialist would follow up within one business day. This is a
                demonstration flow.
              </p>
            ) : null}
          </form>
          ...
          <aside className="border-border h-fit rounded-sm border p-6 lg:sticky lg:top-28">
            ...
            <p className="text-3xl">
              {formatRange(Math.round(total * 0.85), Math.round(total * 1.15))}
            </p>
            <p className="text-muted-foreground mt-3 text-xs leading-relaxed">
              Mock estimate for {qty} window{qty === 1 ? "" : "s"} at roughly {width}″ × {height}″,{" "}
              {mount} mount{motorized ? ", motorised" : ""}. There is no pricing engine behind this
              site — treat it as illustrative only.
            </p>
            <dl className="mt-5 space-y-2 text-xs">
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Area per window</dt>
                <dd>{area.toFixed(1)} sq ft</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Fabric tier</dt>
                <dd className="capitalize">{tier}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Light control</dt>
                <dd className="capitalize">{lining.replace("-", " ")}</dd>
              </div>
            </dl>
          </aside>
        </div>
