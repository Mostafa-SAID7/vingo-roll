...
export const Route = createFileRoute("/services/design-consultation")({
  head: () =>
    pageHead({
      title: "Book a Design Consultation | Vingo Roll",
      description:
        "In-home, virtual or showroom consultation with a window treatment design specialist. Complimentary and no obligation.",
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        description="Sit with a specialist in your home, over video, or in a showroom. We bring fabric, talk light and leave you a written recommendation."
      />
      <Section>
        ...
        {done ? (
          <div className="border-border max-w-xl rounded-sm border p-8" role="status">
            ...
            <p className="text-muted-foreground mt-3 text-sm leading-relaxed">
              Thank you, {form.name}. A specialist would normally confirm your{" "}
              {mode.replace("-", " ")} consultation for {form.date}. This is a demonstration flow —
          ...
          <form onSubmit={onSubmit} noValidate className="grid max-w-3xl gap-6 md:grid-cols-2">
            <fieldset className="md:col-span-2">
              ...
              >
                {[
                  ...
                  <div
                    key={v}
                    className="border-border flex items-center gap-3 rounded-sm border p-4"
                  >
                    <RadioGroupItem value={v!} id={`mode-${v}`} />
                    <Label htmlFor={`mode-${v}`} className="cursor-pointer font-normal">
                      {label}
                    </Label>
                  </div>
                ))}
            ...
            <div>
              ...
              <Select value={form.room} onValueChange={(v) => setForm({ ...form, room: v })}>
                <SelectTrigger id="c-room" className="mt-2">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {rooms.map((r) => (
            ...
            <div className="md:col-span-2">
              ...
              <Select value={form.product} onValueChange={(v) => setForm({ ...form, product: v })}>
                <SelectTrigger id="c-product" className="mt-2">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {categories.map((c) => (
            ...
            <div className="md:col-span-2">
              <Label htmlFor="c-project">Tell us about the project</Label>
              <Textarea
                id="c-project"
                rows={4}
                className="mt-2"
            ...
            {error ? (
              <p role="alert" className="text-destructive md:col-span-2 text-sm">
                {error}
              </p>
            ) : null}
            <div className="md:col-span-2">
